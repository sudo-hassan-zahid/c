...
char **historyEntries = NULL;
size_t historySize = 0;
size_t historyCapacity = INITIAL_CAPACITY;

// void addHistoryEntry(const char *entry) {
//     if (historySize == historyCapacity) {
//         // Resize the array if capacity is exceeded
//         historyCapacity *= 2;
//         historyEntries = (char **)realloc(historyEntries, historyCapacity * sizeof(char *));
...
//     historyEntries[historySize] = (char *)malloc(MAX_STRING_LENGTH * sizeof(char));
//     if (historyEntries[historySize] == NULL) {
//         fprintf(stderr, "Memory allocation failed for entry!\n");
//         exit(1);
//     }

//     // Copy the entry into the allocated memory
//     strncpy(historyEntries[historySize], entry, MAX_STRING_LENGTH - 1);
//     historyEntries[historySize][MAX_STRING_LENGTH - 1] = '\0';  // Null-terminate the string
//     historySize++;
// }
...
// void addHistoryEntryFormatted(double num1, double num2, char op, double result) {
	...
	while (!WindowShouldClose())
	{
		...
		if (GuiButton((Rectangle){420, 600, 80, 180}, "="))
		{
			...
			if (currentOperator)
			{
				...
				addHistoryEntryFormatted(num1, num2, currentOperator, result);
				resultShown = true;
			}
		...
		// History Box Logic
        // Rectangle historyBox = (Rectangle){90, 20, 410, 110};
        DrawRectangleRec(historyBox, LIGHTGRAY);
        DrawRectangleLinesEx(historyBox, 4, BLACK);

        // Ensure scrollIndex is within limits
        if (scrollIndex < 0) scrollIndex = 0;
        if (historySize < historyVisibleCount) scrollIndex = 0;
        else if (scrollIndex > historySize - historyVisibleCount) scrollIndex = historySize - historyVisibleCount;

        // Draw history entries
        if (historySize > 0) {
            for (int i = 0; i < historyVisibleCount; i++) {
                int index = scrollIndex + i;
                if (index < historySize && historyEntries[index] != NULL) {
                    DrawText(historyEntries[index], historyBox.x + 10, historyBox.y + (i * entryHeight) + 5, 22, BLACK);
                }
            }
        } else {
        ...
        // Scroll Down Button
        if (GuiButton((Rectangle){10, 80, 70, 50}, "")) {
            if (scrollIndex < historySize - historyVisibleCount) scrollIndex++;
        }
        DrawText("DOWN", 16, 90, 20, BLACK);
