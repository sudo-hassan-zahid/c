...
char historyEntries[MAX_HISTORY_ENTRIES][200];
int historyCount = 0;
...
void AddHistoryEntry(double val1, double val2, char op, double result)
{
	...
	// {
	//     // If the result is large (>= 1e6) or small (<= 1e-3), format it for scientific notation
	//     sprintf(historyEntry, "%.1f %c %.1f = %.4e", num1, op, num2, result);
	// }
	// else
	// {
	//     // Otherwise, print the result normally (with 2 decimal places)
	//     sprintf(historyEntry, "%.1f %c %.1f = %.2f", num1, op, num2, result);
	// }
	//! working #end
...
double formatSingleResult(double result)
{
	// If the result is large (>= 1e6) or small (<= 1e-3), format it for scientific notation
	if (fabs(result) >= 1e6 || fabs(result) < 1e-3)
	{
		// Return result unchanged for further processing (e.g., in scientific format)
		return result;
	}
	else
	{
		// Return the result as is for regular processing (no change needed)
		return result;
	}
}
...
int main()
{
	...
	while (!WindowShouldClose())
	{
		...
		// Rectangle rec = {20, 300, 80, 80};
		// DrawRectanglePro(rec, {0, 0}, (float)45, BLUE);
		// DrawRectanglePro(
		// 	(Rectangle){20,300,80,80},
		// 	(Vector2){20,30},
		// 	(float)60,
		// 	BLUE
		// );
		...
		if (historyCount > 0)
		{
			for (int i = 0; i < historyVisibleCount; i++)
			{
				...
				{
					DrawText(historyEntries[index], historyBox.x + 30, historyBox.y + (i * entryHeight) + 5, 22, BLACK);
				}
			}
		}
		else
